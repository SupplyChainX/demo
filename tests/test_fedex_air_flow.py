...
def test_api_creates_fedex_air_routes(client):
    payload = {
        "reference_number": "SH-TEST-FEDEX-AIR-001",
        "origin_port": "Tokyo",
    ...
    assert any((route.get('carrier') or '').lower().startswith('fedex') for route in routes)
