...
class TestRouteOptimizerAgent:
    ...
    def test_fetch_and_store_routes(self, mock_for_carrier, app, sample_shipment):
        ...
        with app.app_context():
            ...
            agent = RouteOptimizerAgent()
            routes_created = agent.fetch_and_store_routes(sample_shipment)
            
            assert routes_created == 1
            
            # Verify route was stored in database
            routes = Route.query.filter_by(shipment_id=sample_shipment.id).all()
            assert len(routes) == 1
            
            route = routes[0]
            assert route.shipment_id == sample_shipment.id
    ...
    def test_analyze_route_optimization(self, app, sample_shipment):
        ...
        with app.app_context():
            ...
            agent = RouteOptimizerAgent()
            recommendations = agent._analyze_route_optimization(sample_shipment)
