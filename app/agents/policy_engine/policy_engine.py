...
class PolicyEngine:
    ...
    def evaluate_shipment_policies(self, shipment_data) -> List[PolicyViolation]:
        """Evaluate shipment against all applicable policies"""
        violations = []
        
        try:
            # Handle both Shipment objects and dictionaries
            if hasattr(shipment_data, '__dict__'):
                # Convert Shipment model to dictionary
                context = {
                    "cargo_value": getattr(shipment_data, 'cargo_value_usd', 0) or 0,
                    "route_risk_score": getattr(shipment_data, 'risk_score', 0) or 0,
                    "route_deviation_percent": 0,  # Default value
                    "shipping_mode": getattr(shipment_data, 'transport_mode', 'standard') or 'standard',
                    "cost_premium": 0,  # Default value
                    "risk_score": getattr(shipment_data, 'risk_score', 0) or 0,
                    "active_risk_count": 0  # Default value
                }
            else:
    ...
    def evaluate_supplier_policies(self, supplier_data) -> List[PolicyViolation]:
        """Evaluate supplier against all applicable policies"""
        violations = []
        
        try:
            # Handle both Supplier objects and dictionaries
            if hasattr(supplier_data, '__dict__'):
                # Convert Supplier model to dictionary
                context = {
                    "supplier_score": getattr(supplier_data, 'performance_score', 0) or 0,
                    "relationship_age": 365,  # Default value
                    "compliance_score": getattr(supplier_data, 'compliance_score', 0) or 0,
                    "risk_score": getattr(supplier_data, 'risk_score', 0) or 0,
                    "financial_stability": getattr(supplier_data, 'financial_stability', 'stable') or 'stable'
                }
            else:
                # Handle dictionary input
                context = {
                    "supplier_score": supplier_data.get('performance_score', 0),
                    "relationship_age": supplier_data.get('relationship_age', 365),
                    "compliance_score": supplier_data.get('compliance_score', 0),
                    "risk_score": supplier_data.get('risk_score', 0),
                    "financial_stability": supplier_data.get('financial_stability', 'stable')
                }
            
            # Evaluate supplier-specific policies
            for rule in self.policy_rules[PolicyType.SUPPLIER]:
                violation = self._evaluate_rule(rule, context)
                if violation.violated:
                    violations.append(violation)
                    
            # Evaluate compliance policies for supplier (if defined)
            compliance_rules = self.policy_rules.get(PolicyType.COMPLIANCE, [])
            for rule in compliance_rules:
                violation = self._evaluate_rule(rule, context)
                if violation.violated:
                    violations.append(violation)
                    
            logger.info(f"Evaluated supplier policies: {len(violations)} violations found")
            
        except Exception as e:
            logger.error(f"Error evaluating supplier policies for supplier {getattr(supplier_data, 'id', 'unknown')}: {str(e)}")
            # Return empty violations list on error to prevent cascading failures
